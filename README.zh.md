...
- **填充**：一步以「需要调用工具」结束，于是在工具执行期间把助手这一轮追加进每个成员。这段字节**是**对 adapter 序列化的预测，因此有防护：捕获到的 body 必须能原样通过 `JSON.parse`/`JSON.stringify` 往返，且上一个助手轮只能使用可推理的字段、并具有相同形态（有无 tool calls 要一致）。预测错了会在下一个请求认领时被**逐字节**发现——随后该提供方的填充会被关闭，池退回只靠捕获字节推进。 不同 adapter 对这一轮的**框法不同**——pi-ai 会把工具调用参数从解析后的对象重新序列化，并在这一轮没有文本时**整个省略 `content`**——所以这些框法被**逐一枚举，只发送它们一致的那部分**。预发字节在它们分岔处停下（通常在 `content` 或参数处），剩下的等请求真正到达时**原样写入**。这让填充**不含任何猜测**：它不可能替 adapter 造出一个对方不会构造的 body，因此成员绝不会因为这个被丢弃。代价是覆盖范围——分岔之处正是这轮的**有效载荷**，所以提前发出去的是消息骨架。
- **消费**：下一个请求认领**最老**的那条——只要它的字节能被该请求延续、且请求头仍然一致；其余成员被推进，池再补满。
- **销毁**：一步以**非** `tool-calls` 结束（`stop`、`max-tokens`、报错、中断）时释放整个池——**除非 agent 已经有输入在等**（`agent.inbox.hasPending`：已排队的下一轮，或将在下一步边界投入的输入）。用户已经排过队的回合会重复同一段历史，因此池会被保留并直接使用。前缀不匹配时同样如此：池被丢弃后**立刻用刚捕获到的新前缀重新打开**，所以下一步依然有预传输可用。此外，成员在**空闲** `prewarmHoldMs` 后过期——计时器每次推进都会重置，因此一直随对话推进的成员不会因为「年龄」被退休。
