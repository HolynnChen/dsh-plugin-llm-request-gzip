...
多轮对话每走一步都会把整段历史重发一遍。对某个提供方开启 **预传输** 后，插件会在**一次模型调用发出的那一刻**就打开下一个请求，并把这次调用自带的这段历史写进去——于是上传与模型调用本身、以及随后的工具执行重叠。等下一个请求真正发出时，只需要再写增量（助手这一轮 + 工具结果）。

- **开始**：模型调用一发出就开始。它携带的历史正是下一个请求会重复的那段，所以此刻就已确定，无需猜测 adapter 之后会序列化出什么。
- **保持**：贯穿这次调用、以及它请求的工具执行。
- **销毁**：当一步的结束原因是**非** `tool-calls`（`stop`、`max-tokens`、报错、中断）时立即销毁，因为不会再有请求来消费它；此外还有持有超时、同一会话有更新的调用把它替换、以及超出下面的池上限。
- **上限**：`prewarmPoolSize`（默认 `3`）表示允许多少个会话同时持有。每个会话永远只存在**一个**可能的后继请求，所以这是对并发挂起请求的资源约束，而不是一组可供挑选的备选。

默认关闭，而且需要说清为什么它是一次需要实测的实验、而不是白捡的优化：

- **收益上限就是它挪走的那段上传。** 「发送」列就是这段上传；在这套部署上是几十到几百毫秒，而单个请求是几十秒量级。它只是把这段挪出关键路径；别的动不了——网关仍然必须收完整个请求体才会开始推理。
- **挂起的请求会占用一个网关槽位**，且要占用整个工具执行期。body 超时较短的网关会把它丢掉——无害，但这一步也就没有收益。
- **它会产出 chunked 请求体**，部分中转站会拒绝。

因此每次尝试都有保护和可观测量：

- 只有当到达的请求体**确实以预发字节为前缀**、且**所有请求头仍然一致**时才会复用那个挂起请求；任何不一致都会丢弃它，按普通请求发出。
- 如果该 endpoint 返回异常——或拒绝 chunked 请求体——预传输会在本进程内对该 endpoint 关闭；其中「形状类」拒绝（411/415/501）会立即按普通请求重发。
- 机制内部任何失败都会回退为普通请求。
- 用上它的行会带 **预热** 标记，悬停可以看到预发字节数、增量字节数和持有时间。

gzip 与预传输可以并存：切分时保持**同一个 deflate 流**不关闭，因此两段能作为**一个整体**解压，压缩不会因为预传输而被牺牲。
