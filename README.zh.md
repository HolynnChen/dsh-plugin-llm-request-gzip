...
| 缓存 | 提示词中由提供方前缀缓存直接命中的比例（缓存读取 ÷ 输入），悬停可看原始 token 数。 |
...
多轮对话每走一步都会把整段历史重发一遍。对某个提供方开启 **预传输** 后，插件会在一步结束时立刻把**下一个请求的共享历史**发出去——此时这一步要调用的工具还在跑——于是下一个请求真正发出时，只需要再写增量（助手这一轮 + 工具结果）。
