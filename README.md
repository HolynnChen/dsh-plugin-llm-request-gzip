...
The plugin carries a three-part version (`package.json`, currently `1.1.1`), and the settings card shows it with a button. **检查更新** asks the Host for the version published on the repository's `main` branch and compares the two; when the published one is newer the button becomes **更新到 X**.
