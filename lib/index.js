...
export function apply(ctx) {
	...
	const claimPrewarm = (scope, input, init) => {
		...
		let chosen;
		let filled = false;
		if (init !== null && typeof init === "object" && typeof init.body === "string") {
			...
			if (scope !== undefined && scope.prewarm === true) {
				...
				if (claimed !== undefined) {
					...
					if (response !== undefined) {
						// The request was served from a held member, so nothing is sent —
						// but the capture still has to be recorded (for the next pool
						// generation) and the pool brought up to date for the step after.
