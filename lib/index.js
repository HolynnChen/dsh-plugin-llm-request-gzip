...
import { ASSISTANT_VARIANTS, headersMatch, isShapeRejection, predictAssistantIncrement, prewarmPrefix } from "./prewarm.js";
import { createTimingStore } from "./timing.js";

/**
 * Encode one request body. Brotli is preferred when the runtime has it — on the
 * JSON these adapters send it is typically 15–25% smaller than gzip — and it is
...
export function apply(ctx) {
	...
	/**
	 * The framing convention each provider turned out to use, learned from the
	 * request that consumed a filled member. Adapters differ — pi-ai re-serializes
	 * tool-call arguments from the parsed object and omits `content` on a turn with
	 * no text — and the differences are not something to assume, so until one is
	 * learned the pool carries a variant each and the request itself decides.
	 */
	...
	const fillAssistant = (scope, assistant) => {
		...
		const learned = scope.provider === undefined ? undefined : assistantVariants.get(scope.provider);
		const candidates = [];
		for (const variant of learned === undefined ? ASSISTANT_VARIANTS : [learned]) {
			const predicted = predictAssistantIncrement(scope.capture.body, assistant, variant);
			if (predicted !== undefined) candidates.push({ variant, predicted });
		}
		if (candidates.length === 0) return;
		let pick = 0;
		for (const member of membersOf(key)) {
			if (member.settled === true) continue;
			// Only members sitting exactly at the frontier can take the turn.
			const usable = candidates.filter((candidate) => candidate.predicted.from === member.prefix);
			if (usable.length === 0) continue;
			const choice = usable[pick++ % usable.length];
			appendMember(member, choice.predicted.appended).then((ok) => {
				if (ok) {
					member.prefix = choice.predicted.prefix;
					member.variant = choice.variant;
					member.filled = true;
					refreshHold(member);
	...
	const claimPrewarm = (scope, input, init) => {
		...
		if (chosen === undefined) {
			...
			if (filled && scope.provider !== undefined) {
				...
				ctx.logger?.warn?.("model-request-accelerator: the predicted assistant turn did not match; pre-transmission now advances on captured bytes only");
			}
			return miss(filled ? "fill" : "mismatch");
		}
		// Whichever framing this member carried is the one the adapter uses, so the
		// next fill can stop hedging and just use it.
		if (chosen.variant !== undefined && scope.provider !== undefined && assistantVariants.get(scope.provider) !== chosen.variant) {
			assistantVariants.set(scope.provider, chosen.variant);
			ctx.logger?.info?.(`model-request-accelerator: ${scope.provider} frames the assistant turn as "${chosen.variant}"`);
		}
	...
	ctx.on("llm/stream", (options, next) => {
		...
		/**
		 * Whether the agent already has input waiting — a queued next turn, or input
		 * due at the next step boundary. Either way another request follows, and it
		 * repeats this history verbatim, so a pool that is discarded here would only
		 * be rebuilt on the next request anyway.
		 */
		const hasPendingInput = () => {
			if (scope.sessionId === null) return false;
			try {
				return ctx.get("agents")?.get?.(scope.sessionId)?.inbox?.hasPending === true;
			} catch {
				return false;
			}
		};
		/**
		 * The assistant turn belongs to the next request whenever one follows: a step
		 * that asked for tools, or a turn boundary the user has already queued past.
		 * Only a turn that ends with nothing waiting releases the pool.
		 */
		const settlePrewarm = () => {
			if (finishKind !== "tool-calls" && !hasPendingInput()) {
				dropPoolFor(scope);
				return;
			}
			if (finishKind !== "tool-calls") ctx.logger?.debug?.("model-request-accelerator: keeping the pool — input is already waiting");
	...
	const planRequest = (input, init, options = {}) => {
		...
		if (store !== undefined && store !== null && isModelBody && store.capture === undefined) {
			store.capture = { body: init.body, headers: plainHeaders(init.headers), url, encoding: undefined };
		}
		...
		if (options.compress === false && isModelBody && store?.capture !== undefined && store.capture.body === init.body) store.capture.encoding = preferred;
		if (isModelBody && store?.capture !== undefined && store.capture.body === init.body) store.capture.encoding = plan === undefined ? undefined : plan.encoding;
		if (measured) timing.noteFetch(record, url, Buffer.byteLength(init.body, "utf8"));
		if (plan === undefined) return undefined;
		if (measured) timing.noteSent(record, plan.compressedBytes, plan.encoding);
		return {
			...plan,
			provider: store?.provider,
			request: { input, init },
			init: { ...init, headers: withEncodingHeader(headers, plan.encoding), body: plan.body }
		};
	};
		...
		try {
			...
			if (scope !== undefined && scope.prewarm === true) {
				...
				if (claimed !== undefined) {
					...
					planRequest(input, init, { compress: false });
