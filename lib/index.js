...
export function apply(ctx) {
	...
	const state = { policies: new Map(), byEndpoint: new Map(), timing: true, prewarmHoldMs: DEFAULT_PREWARM_HOLD_MS };
	let source = () => BASE;
	...
	ctx.on("llm/stream", (options, next) => {
		...
		/** A stream only offers a prefix to pre-send once it has finished cleanly. */
		let sawFinish = false;
		const maybePrewarm = (clean) => {
			if (scope.prewarm !== true || scope.capture === undefined) return;
			if (clean !== true && sawFinish !== true) return;
			...
			[Symbol.asyncIterator]() {
				...
				return {
					next: async (...args) => {
						...
						if (result.value !== null && typeof result.value === "object" && result.value.type === "finish") sawFinish = true;
