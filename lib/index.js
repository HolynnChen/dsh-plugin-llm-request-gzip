...
export function apply(ctx) {
	...
	const noteEndpointFailure = (url, status) => {
		if (isShapeRejection(status)) {
			...
			ctx.logger?.warn?.(`llm-request-gzip: ${url} answered ${status} to a pre-transmitted request; pre-transmission is off for it`);
			return;
		}
		const count = (endpointFailures.get(url) ?? 0) + 1;
		endpointFailures.set(url, count);
		if (count < MAX_ENDPOINT_FAILURES) {
			ctx.logger?.warn?.(`llm-request-gzip: ${url} answered ${status} to a pre-transmitted request (${count}/${MAX_ENDPOINT_FAILURES})`);
			return;
		}
		prewarmBlocked.add(url);
		endpointFailures.delete(url);
		ctx.logger?.warn?.(`llm-request-gzip: ${url} answered ${status} ${count} times in a row to pre-transmitted requests; pre-transmission is off for it`);
	};
	...
	ctx.on("llm/stream", (options, next) => {
		...
		const settlePrewarm = () => {
			...
			if (finishKind !== "tool-calls") ctx.logger?.debug?.("llm-request-gzip: keeping the pool — input is already waiting");
	...
	const patchedFetch = async function patchedFetch(input, init) {
		...
		ctx.logger?.info?.(`llm-request-gzip: ${planned.provider ?? "endpoint-matched"} request compressed with ${planned.encoding} ${planned.originalBytes} -> ${planned.compressedBytes} bytes`);
		...
		ctx.logger?.warn?.(`llm-request-gzip: ${url} answered ${response.status} to a brotli body; using gzip for it from now on`);
		try {
