...
export function apply(ctx) {
	 ...
	 * call, so the `fetch` patch can tell two routes on one endpoint apart.
	 * Each iterator resumption runs inside the scope, because the adapters await
	 * (image serialization, file upload) before they reach `fetch`.
	 */
	ctx.on("llm/stream", (options, next) => {
		const inner = next();
		const provider = options === null || typeof options !== "object" ? undefined : options.provider;
		...
		return {
			[Symbol.asyncIterator]() {
				...
				const scoped = (run) => attribution.run({ provider }, run);
				return {
					next: (...args) => scoped(() => iterator.next(...args)),
					return: (value) => typeof iterator.return === "function" ? scoped(() => iterator.return(value)) : Promise.resolve({ done: true, value }),
					throw: (error) => typeof iterator.throw === "function" ? scoped(() => iterator.throw(error)) : Promise.reject(error)
				};
			}
		};
	});
	...
	const planRequest = (input, init) => {
		...
		const policy = resolvePolicy({
			url,
			provider: attribution.getStore()?.provider,
