...
import { headersMatch, isShapeRejection, predictAssistantCommonIncrement, prewarmPrefix } from "./prewarm.js";
...
export function apply(ctx) {
	...
	const fillAssistant = (scope, assistant) => {
		const key = prewarmKey(scope);
		if (key === undefined || scope.capture === undefined) return Promise.resolve();
		if (scope.provider !== undefined && assistantFillDisabled.has(scope.provider)) return Promise.resolve();
		return withPoolLock(key, () => fillAssistantLocked(key, scope, assistant));
	};

	/** The fill itself, called only while holding the conversation's lock. */
	const fillAssistantLocked = async (key, scope, assistant) => {
		// Only the bytes every framing agrees on, so a member can never be sent a
		// body the adapter would not have built. The request that follows writes
		// the rest verbatim.
		const predicted = predictAssistantCommonIncrement(scope.capture.body, assistant);
		if (predicted === undefined) return;
		for (const member of membersOf(key)) {
			if (member.settled === true || member.prefix !== predicted.from) continue;
			if (await appendMember(member, predicted.appended)) {
	...
	ctx.inject(["connection"], (connectionCtx) => {
		connectionCtx.connection.fetch.register({
			...
			fetch: async (request) => {
				...
				if (ledger !== undefined && !timing.has(sessionId)) timing.seed(sessionId, ledger.read(sessionId));
	...
	const planRequest = (input, init, options = {}) => {
		...
		if (isModelBody && store?.capture !== undefined && store.capture.body === init.body) {
