 ...
 *
 * Splits one model call into the phases a client can actually act on, from two
 * independent sources — undici's transport diagnostics and the adapter's own
 * chunk stream:
 *
...
/** Most recent measurements kept in memory per session. The ledger holds more. */
export const MAX_PER_SESSION = 100;

/** Most recent sessions kept in memory before the oldest is evicted. */
...
export function createTimingStore(options) {
	 ...
	 */
	const notePrewarmMiss = (record, reason, detail) => {
		record.prewarmMiss = reason;
		if (detail !== undefined && detail !== null) record.prewarmMissDetail = detail;
	};
	...
	/**
	 * Add one received response chunk. These are wire bytes, so a compressed reply
	 * counts as what actually crossed the network.
	...
	/**
	 * Remember how the response body was encoded on the wire, so a compressed
	...
	/**
	 * Remember which protocol carried the response.
	 *
	 * This is read from the transport's own answer rather than from the fact that
	 * HTTP/2 was requested, because an endpoint that does not offer h2 through ALPN
	 * answers on http/1.1 with no error at all — so the two cases are only
	 * distinguishable by what came back.
	 *
	 * @param record - the measurement.
	 * @param protocol - the transport's report, e.g. `h2` or `h1`.
	 */
	const noteProtocol = (record, protocol) => {
		if (typeof protocol !== "string") return;
		const trimmed = protocol.trim();
		if (trimmed.length > 0) record.protocol = trimmed;
	};
	...
	const summarize = (record) => {
		...
		return {
			...
			prewarmMissDetail: record.prewarmMissDetail,
	...
	const seed = (sessionId, rows) => {
		const key = sessionId ?? "";
		if (restored.has(key)) return false;
		if (!Array.isArray(rows) || rows.length === 0) return false;
		// Live records for this session may already exist — the panel is opened
		// after the session has run something — and the stored rows still belong in
		// front of them. Ids continue past the stored ones so the merged history
		// stays strictly increasing.
		for (const row of rows) if (Number.isFinite(row?.id) && row.id >= nextId) nextId = row.id + 1;
		// A live record may already hold one of these ids — the store opens
		// asynchronously, so the first call of a session can be numbered before its
		// history is found. Renumber those past the restored ones rather than
		// serving two rows with the same id.
		for (const live of sessions.get(key) ?? []) {
			if (typeof live.id === "number" && live.id < nextId) live.id = nextId++;
		}
	...
	/** Forget every record held in memory. The durable store is cleared separately. */
	const clear = () => {
		sessions.clear();
		restored.clear();
	};

	/** What is held in memory right now, for the settings card. */
	const stats = () => {
		let rows = 0;
		for (const list of sessions.values()) rows += list.length;
		for (const list of restored.values()) rows += list.length;
		const keys = new Set([...sessions.keys(), ...restored.keys()]);
		return { sessions: keys.size, rows };
	};
	...
	return { begin, seed, has, clear, stats, noteFetch, noteSent, noteProtocol, claimsRequest, notePhase, noteResponseBytes, noteResponseEncoding, notePrewarm, notePrewarmMiss, observeChunk, finish, summarize, snapshot };
}
