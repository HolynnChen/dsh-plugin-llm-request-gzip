...
export function createTimingStore(options) {
	 ...
	 */
	const noteSent = (record, sentBytes) => {
	...
	const summarize = (record) => {
		...
		return {
			...
			cacheHitPercent: record.cacheReadTokens !== null && record.inputTokens !== null && record.inputTokens > 0
				? round((record.cacheReadTokens / record.inputTokens) * 100)
				: null,
	...
	return { begin, noteFetch, noteSent, claimsRequest, notePhase, noteResponseBytes, noteResponseEncoding, notePrewarm, observeChunk, finish, summarize, snapshot };
}
