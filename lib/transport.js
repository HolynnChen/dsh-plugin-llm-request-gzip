...
/**
 * Where a peer copy of undici may live, nearest first. The plugin's own
 * `node_modules` wins when a deployment installed its dependencies explicitly;
 * the profile's hoisted directory is where dsh's own dependency tree puts it,
 * which is the case this plugin normally runs in.
 */
...
function loadUndici() {
	for (const candidate of UNDICI_CANDIDATES) {
		try {
			const loaded = createRequire(import.meta.url)(join(candidate, "index.js"));
	...
	return {
		...
		async execute(input, init, options = {}) {
			...
			const watch = typeof options.onConnected === "function" ? (message) => {
				...
				const expectedPort = target === undefined || target.port === ""
					? target === undefined ? undefined : target.protocol === "https:" ? "443" : "80"
					: target.port;
			...
			if (announced !== undefined) options.onConnected(announced);
