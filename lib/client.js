...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const TS = {
			...
			// The rows scroll inside the panel and the header stays pinned to the top
			// of this box. The viewport bound is what makes that work: while a session
			// is active the shell lays the view area out with `min-height: auto`, so
			// the panel's `height: 100%` resolves against an auto height and, without
			// this, the page itself becomes the scroller and the header would pin to
			// the top of the transcript instead of to the table. `maxHeight` never
			// forces the box open, so a shorter flex parent still wins.
			tableWrap: {
				flex: "1 1 auto",
				minHeight: 0,
				overflow: "auto",
		...
		function TimingView(props) {
			...
			return h("div", { ref: rootRef, style: TS.root }, children);
		}
