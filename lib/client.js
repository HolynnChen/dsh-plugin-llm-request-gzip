...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const S = {
			...
			gridProviders: {
				display: "grid",
				gridTemplateColumns: "minmax(0, max-content) 34px 72px",
				alignItems: "center",
		...
		function routeCells(props) {
			...
			return [
				h("input", {
					...
					style: { ...S.cellCheck, order: 1 },
					onChange: (event) => props.onToggle(route.id, event.target.checked)
				}),
				h("span", { key: `${route.id}:name`, style: { ...S.identity, order: 0 } },
		...
		function SettingsCard(props) {
			...
			if (state.snapshot === null) {
				...
				const cells = [
					h("span", { key: "h-gzip", style: { ...S.colHead, order: 1 }, title: COPY.compressHint }, COPY.columnCompress),
					h("span", { key: "h-name", style: { ...S.colHead, order: 0 } }, COPY.columnProvider),
					h("span", { key: "h-prewarm", style: { ...S.colHead, order: 2 }, title: COPY.prewarmHint }, COPY.prewarm),
					h("span", { key: "h-size", style: { ...S.colHead, display: "none" }, title: COPY.advancedHint }, COPY.columnSize)
				];
