...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const TIMING_COPY = {
			label: "请求耗时",
			intro: "按模型请求拆解耗时。发送 = 请求体上传完毕；首 token = 发送完毕到首个 token；生成 = 首个 token 到结束，tok/s 按该区间计算。",
			loading: "正在读取…",
		...
		function TimingView(props) {
			...
			for (let index = measurements.length - 1; index >= 0; index--) {
				...
				const cells = [
					...
					h("td", { key: "r", style: { ...TS.td, ...TS.tdLeft } },
						...
						item.compressed === null ? null : h("span", { style: TS.chip }, "gzip")
					),
					h("td", { key: "p", style: { ...TS.td, ...TS.dim } }, formatMs(item.prepareMs)),
					...
					h("td", { key: "o", style: TS.td }, item.outputTokens === null ? "–" : String(item.outputTokens)),
					...
					h("td", {
						key: "x",
						style: { ...TS.td, ...(item.status === "error" ? TS.errorText : TS.dim) }
					}, item.status === "running" ? TIMING_COPY.running : item.status === "error" ? TIMING_COPY.error : "")
				];
				rows.push(h("tr", { key: String(item.id) }, cells));
			}
			...
			return h("div", { style: TS.root },
				...
				h("p", { style: TS.intro }, TIMING_COPY.intro),
				state.error === null ? null : h("p", { style: { ...TS.intro, ...TS.errorText } }, state.error),
				measurements.length === 0
					? h("p", { style: TS.intro }, state.status === "loading" ? TIMING_COPY.loading : TIMING_COPY.empty)
					: h("table", { style: TS.table },
						h("thead", null, h("tr", null,
							h("th", { style: { ...TS.th, ...TS.thLeft } }, TIMING_COPY.column.time),
							h("th", { style: { ...TS.th, ...TS.thLeft } }, TIMING_COPY.column.route),
							h("th", { style: TS.th }, TIMING_COPY.column.prepare),
							h("th", { style: TS.th }, TIMING_COPY.column.send),
							h("th", { style: TS.th }, TIMING_COPY.column.server),
							h("th", { style: TS.th }, TIMING_COPY.column.ttft),
							h("th", { style: TS.th }, TIMING_COPY.column.generation),
							h("th", { style: TS.th }, TIMING_COPY.column.output),
							h("th", { style: TS.th }, TIMING_COPY.throughput),
							h("th", { style: TS.th }, TIMING_COPY.column.total),
							h("th", { style: TS.th }, "")
						)),
						h("tbody", null, rows)
					)
