...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const COPY = {
			title: "模型请求 gzip 与耗时",
			description: "按提供方压缩请求体，并展示请求耗时分解。",
			intro:
		...
		function SettingsCard(props) {
			...
			const save = React.useCallback(async (target, patch) => {
				...
				try {
					...
					setNotice(target === "timing" ? "请求耗时设置已保存" : `${target} 已保存`);
					reload();
				} catch (error) {
		...
		const TIMING_COPY = {
			...
			hint: {
				...
				response: "响应在线路上的字节数（网关压缩后的实际传输量）",
				total: "发出请求 → 流结束；括号内是准备耗时与输出 token 数"
			},
			running: "进行中",
			error: "失败"
		};
		...
		/**
		 * The conversation column's width handles are shell chrome, absolutely
		 * positioned in the gutters with `z-index: 8` for every view that is active
		 * on a session. A view cannot un-render them, so the panel instead covers
		 * exactly their two bands: pointer events then land on the panel, and the
		 * timing table cannot be resized by a stray drag. Both bands are outside
		 * the centred content column, so no content sits under them.
		 */
		const SHIELD_WIDTH = "min(40px, calc((100% - var(--dsh-chat-content-width, 680px)) / 2 - 48px))";
		...
		const TS = {
			...
			shieldLeft: {
				...
				zIndex: 9,
				width: SHIELD_WIDTH,
				right: "calc(50% + var(--dsh-chat-content-width, 680px) / 2 + 24px)"
			},
			shieldRight: {
				position: "absolute",
				top: 0,
				...
				zIndex: 9,
				width: SHIELD_WIDTH,
				left: "calc(50% + var(--dsh-chat-content-width, 680px) / 2 + 24px)"
			},
			...
			// The table scrolls inside the panel rather than widening it, so a narrow
			// window never turns the view into a horizontally scrolling surface.
			tableWrap: { flex: "1 1 auto", minHeight: 0, overflow: "auto" },
			table: { borderCollapse: "collapse", width: "100%", fontSize: "12px", tableLayout: "fixed" },
			th: {
				textAlign: "right",
				padding: "4px 6px",
				color: "var(--dsw-alias-label-tertiary)",
		...
		/** Fixed column widths keep the table from being sized by its own content. */
		const COLUMN_WIDTHS = ["62px", "auto", "70px", "70px", "78px", "70px", "64px", "auto", "78px", "auto"];
		...
		function rowDetail(item) {
			...
			if (item.responseBytes > 0) parts.push(`响应 ${formatBytes(item.responseBytes)}`);
			return parts.join(" · ");
		}
		...
		function TimingView(props) {
			...
			const headerCells = [
				h("th", { key: "time", style: { ...TS.th, ...TS.thLeft } }, columns.time),
				h("th", { key: "route", style: { ...TS.th, ...TS.thLeft } }, columns.route),
			...
			for (let index = measurements.length - 1; index >= 0; index--) {
				...
				rows.push(h("tr", { key: String(item.id), title: detail === "" ? undefined : detail },
					...
					h("td", { key: "p", style: TS.td, title: TIMING_COPY.hint.response }, formatBytes(item.responseBytes)),
			...
			children.push(measurements.length === 0
				...
				: h("div", { key: "table", style: TS.tableWrap },
					h("table", { style: TS.table },
						h("colgroup", null, COLUMN_WIDTHS.map((width, index) => h("col", { key: `c${String(index)}`, style: { width } }))),
						h("thead", null, h("tr", null, headerCells)),
						h("tbody", null, rows)
					)
				));
			return h("div", { style: TS.root },
				h("div", { key: "shield-left", style: TS.shieldLeft, "data-handle-shield": "left", "aria-hidden": "true" }),
				h("div", { key: "shield-right", style: TS.shieldRight, "data-handle-shield": "right", "aria-hidden": "true" }),
				children
			);
		}
