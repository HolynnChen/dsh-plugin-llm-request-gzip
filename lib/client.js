...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const COPY = {
			...
			endpointClean: "压缩与预传输均未被该 endpoint 拒绝过（以下状态来自真实请求，非探测）",
			...
			columnCompress: "压缩",
			columnSize: "最小体积",
			compressHint: "勾选 = 该提供方的请求体压缩后上传，算法由上面的「压缩算法」决定",
			encoding: "压缩算法",
		...
		function SettingsCard(props) {
			...
			if (state.snapshot === null) {
				...
				const cells = [
					h("span", { key: "h-gzip", style: S.colHead, title: COPY.compressHint }, COPY.columnCompress),
				...
				for (const group of groups) {
					if (group.endpoint !== null) {
						...
						(() => {
							...
							return h("span", { style: S.muted }, notes.length === 0 ? COPY.endpointClean : notes.join(" · "));
						})(),
