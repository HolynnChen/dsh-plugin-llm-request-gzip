...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const S = {
			...
			// Plugin-level settings: label column, control column, one shared edge.
			gridSettings: {
				display: "grid",
			...
			settingRow: { display: "inline-flex", alignItems: "flex-start", gap: "8px", cursor: "pointer", minWidth: 0 },
			settingText: { flexDirection: "column", gap: "2px", display: "flex", minWidth: 0 },
			settingLabel: { fontSize: "13px", lineHeight: "20px", color: "var(--dsw-alias-label-primary)" },
			cellCheck: { margin: 0, marginTop: "2px", flex: "none" },
			// Providers: one grid with headings, so columns align across every row.
			gridProviders: {
				display: "grid",
				gridTemplateColumns: "34px minmax(0, 1fr) 68px 104px",
				alignItems: "center",
				gap: "7px 10px"
			},
			colHead: {
			...
			groupNote: { margin: 0, fontSize: "11.5px", lineHeight: "17px", color: "var(--dsw-alias-state-warn-label)" },
			groupHead: { display: "flex", flexDirection: "column", gap: "3px", minWidth: 0 },
			row: {
				display: "flex",
				alignItems: "center",
				gap: "10px",
		...
		/**
		 * One provider's four grid cells, in table order: the gzip switch, the
		 * identity, the pre-transmission switch, and the size threshold. Cells
		 * rather than a row, so every provider lines up under the same column
		 * headings however long its name is.
		 */
		function routeCells(props) {
			...
			const onBlur = (event) => {
				...
				props.onThreshold(route.id, parsed);
			};
			return [
				h("input", {
					key: `${route.id}:gzip`,
					...
					disabled,
					title: `${route.id}：${COPY.advancedHint}`,
					style: S.cellCheck,
				...
				h("input", {
					key: `${route.id}:minBytes:${String(route.minBytes)}`,
					type: "number",
					min: 0,
					step: 1,
					defaultValue: String(route.minBytes),
					disabled: disabled || !route.enabled,
					style: { ...S.input, opacity: route.enabled ? 1 : 0.45, justifySelf: "end" },
		...
		function SettingsCard(props) {
			...
			if (state.snapshot === null) {
				...
				// Plugin-level settings on a grid, so every control shares one edge.
				children.push(h("div", { key: "settings", style: S.gridSettings },
					h("label", { key: "timing-label", style: S.settingRow },
						h("input", {
							type: "checkbox",
						...
						h("span", { style: S.settingText },
							h("span", { style: S.settingLabel }, COPY.timing),
							h("span", { style: S.muted }, COPY.timingHint)
						)
					),
					h("span", { key: "timing-empty" }),
					h("label", { key: "pool-label", style: S.settingRow, title: COPY.poolHint },
						h("span", { style: S.settingText },
				...
				// Providers: one grid with a heading row, so the columns line up
				// across every route and every endpoint group.
				const groups = groupRoutes(snapshot.routes);
				children.push(h("div", { key: "providers", style: S.section },
					h("div", { key: "head", style: S.gridProviders },
						h("span", { key: "h-gzip", style: S.colHead, title: COPY.advancedHint }, "gzip"),
						h("span", { key: "h-name", style: S.colHead }, COPY.columnProvider),
						h("span", { key: "h-prewarm", style: S.colHead, title: COPY.prewarmHint }, COPY.prewarm),
						h("span", { key: "h-size", style: { ...S.colHead, textAlign: "right" }, title: COPY.advancedHint }, COPY.columnSize)
					),
					snapshot.routes.length === 0 ? h("p", { key: "empty", style: S.muted }, COPY.empty) : null,
					groups.map((group, index) => h("div", { key: `group-${String(index)}`, style: S.group },
						group.endpoint === null ? null : h("div", { key: "head", style: S.groupHead },
							h("span", { key: "url", style: S.endpoint }, group.endpoint),
							group.routes.length > 1 && group.routes.some((route) => route.enabled)
								? h("span", { key: "note", style: S.groupNote }, COPY.sharedEndpoint)
								: null
						),
						h("div", { key: "grid", style: S.gridProviders },
							group.routes.flatMap((route) => routeCells({
								route,
								busy: busy === route.id,
		 ...
		 * 发送, 服务端, 首token, 生成, tok/s, 缓存, 请求体, 响应体, 总计.
