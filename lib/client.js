...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const COPY = {
			...
			intro:
				"响应侧 gzip 由 Node 默认开启；这里控制的是请求体压缩——对指定提供方以 content-encoding: gzip 上传，长上下文收益最大。",
			timing: "展示「请求耗时」面板",
			timingHint: "在会话视图切换器里与「轨迹」并列，关闭后不再记录耗时。",
			sharedEndpoint: "共用同一 endpoint，开关会同时作用于它们：",
			loading: "正在读取设置…",
			retry: "重试",
			empty: "当前没有已注册的提供方路由。",
		...
		const S = {
			...
			settings: { display: "flex", flexDirection: "column", gap: "13px" },
			...
			gridProviders: {
				display: "grid",
				gridTemplateColumns: "34px max-content 62px 104px",
				alignItems: "center",
