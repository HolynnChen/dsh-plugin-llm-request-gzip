...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const TIMING_COPY = {
			...
			hint: {
				time: "该请求发出的本地时刻",
				route: "提供方路由与模型；gzip 表示请求体确实被压缩过",
				send: "发出请求 → 请求体全部发送完毕",
				server: "发送完毕 → 收到响应头",
				ttft: "发送完毕 → 首个 token",
		 ...
		 * 发送, 服务端, 首 token, 生成, tok/s, 请求体, 响应体, 总计. The free-text column
		 ...
		 */
		const COLUMN_SHARES = ["7%", "30%", "6%", "6%", "7%", "6%", "6%", "12%", "10%", "10%"];
		...
		/**
		 * The handles span the whole conversation body, which extends past this
		 * panel and down beside the composer. The shields therefore reach well
		 * below the panel; the band they cover is the gutter, so nothing else is
		 * underneath it.
		 */
		const SHIELD_HEIGHT = "calc(100% + 520px)";
		...
		const TS = {
			...
			// handed the widest value (the size pair) the widest column.
