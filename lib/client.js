...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		 ...
		 * 发送, 服务端, 首 token, 生成, tok/s, 缓存, 请求体, 响应体, 总计. The free-text column
		 ...
		 */
		const COLUMN_SHARES = ["6%", "26%", "6%", "6%", "7%", "6%", "6%", "6%", "11%", "10%", "10%"];
		...
		function rowDetail(item) {
			...
			if (item.prewarm !== null && item.prewarm !== undefined) {
				parts.push(`预传输：历史 ${item.prewarm.prefixBytes} 字节 / 增量 ${item.prewarm.deltaBytes} 字节 / 持有 ${item.prewarm.holdMs}ms`);
			}
