...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const COPY = {
			...
			intro:
				"响应侧压缩由 Node 默认开启；这里控制的是请求体：对指定提供方以 content-encoding: br（优先）或 gzip 上传，长上下文的上传耗时收益最大。",
			timing: "展示「请求耗时」面板",
			...
			encodingGzip: "仅 gzip",
			encodingHint: "br 在同样耗时下比 gzip 再小 5~15%；运行时没有 br、或该 endpoint 拒绝 br 时自动退回 gzip。",
		...
		const S = {
			...
			gridProviders: {
				display: "grid",
				gridTemplateColumns: "34px minmax(0, max-content) 62px 104px",
				alignItems: "center",
		...
		function routeCells(props) {
			...
			return [
				...
				h("input", {
					...
					style: { ...S.input, textAlign: "right", opacity: route.enabled ? 1 : 0.45, justifySelf: "end" },
		...
		function SettingsCard(props) {
			...
			if (state.snapshot === null) {
				...
				const cells = [
					...
					h("span", { key: "h-size", style: { ...S.colHead, justifySelf: "end" }, title: COPY.advancedHint }, COPY.columnSize)
				];
				for (const group of groups) {
					if (group.endpoint !== null) {
						cells.push(h("div", { key: `${group.endpoint}:head`, style: S.groupHead },
							h("span", { style: S.endpoint }, group.endpoint),
							group.routes.length > 1 && group.routes.some((route) => route.enabled)
								? h("span", { style: S.groupNote }, COPY.sharedEndpoint)
								: null
						));
					}
		...
		const PREWARM_MISS = {
			...
			mismatch: "没有任何成员的字节能被这次请求延续——历史发生了变化（例如压缩或重写）",
			fill: "助手轮字节预测不匹配，池已丢弃（该提供方的预测填充已关闭）",
			shape: "这次请求的 body 形态不适合预传输",
			failed: "已领用的预发请求在交接时断了（通常是网关把长时间空闲的连接关掉），本次只能整段重传",
			aborted: "请求在领用后被取消",
		...
		function rowDetail(item) {
			...
			if (item.prewarm !== null && item.prewarm !== undefined) {
				...
				const tail = item.prewarm.tailBytes === undefined || item.prewarm.tailBytes === null || item.prewarm.tailBytes <= 0
					? ""
					: `；其中 ${formatBytes(item.prewarm.tailBytes)} 在 messages 之后（tools 等），无法预发`;
