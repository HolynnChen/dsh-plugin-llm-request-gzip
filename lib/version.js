 ...
 *
 * The published version lives in `package.json` and in the repository's tags;
 * the running version is read from the installed `package.json`. An update is a
 * fast-forward pull in the plugin's own directory — the same thing the installer
 * does — so the panel is only ever reporting and triggering what a manual
 * `git pull` would have done.
 *
