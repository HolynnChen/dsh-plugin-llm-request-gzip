...
export async function openLedger(storage, log) {
	...
	return {
		...
		/**
		 * How much the stored records occupy and how many sessions they cover.
		 * Serialized rather than asked of the backend, because the backend stores
		 * its own envelope around them and this is an estimate either way.
		 * @returns the totals.
		 */
		stats() {
			let bytes = 0;
			for (const [, value] of sessions.entries()) bytes += Buffer.byteLength(JSON.stringify(value), "utf8");
			return { sessions: sessions.size, bytes };
		},
		/**
		 * Delete every stored session, including ones this process never loaded.
		 * @returns how many documents were removed.
		 */
		async clear() {
			let removed = 0;
			for (const key of [...sessions.keys()]) if (await sessions.delete(key)) removed += 1;
			return removed;
		},
		/** Release the domain. */
		close() {
